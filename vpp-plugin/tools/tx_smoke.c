...
main (int argc, char **argv)
{
  ...
  for (uint64_t request_id = 1; request_id <= count; request_id++)
    {
      ...
      do
	rc = uet_vpp_client_acquire_dma (client, &slot, &packet, &capacity);
      while (rc == -EAGAIN && monotonic_ns () < deadline);
      if (rc || !(packet_length = packet_build (packet, capacity, request_id)))
	{
	  fprintf (stderr, "TX slot acquisition/build failed for request %" PRIu64 ": %d\n",
		   request_id, rc);
	  goto done;
	}

      rc = uet_vpp_client_submit_ip (client, slot, packet_length, request_id, request_id ^ count);
      if (rc)
	{
	  fprintf (stderr, "TX submit failed for request %" PRIu64 ": %d\n", request_id, rc);
	  uet_vpp_client_release_dma (client, slot);
	  goto done;
	}
      ...
      do
	{
	  ...
	  completion.request_id != request_id || completion.user_context != (request_id ^ count))
	{
	  fprintf (stderr, "invalid TX completion for request %" PRIu64 "\n", request_id);
	  goto done;
	}
      deadline = monotonic_ns () + UET_TX_SMOKE_TIMEOUT_NS;
    }
  ...
  printf ("UET routed TX smoke passed: %" PRIu64
	  " packets, IPv4/IPv6 native and UDP, %u replacement slots\n",
	  count, info.dma_slot_count);
  rv = 0;
